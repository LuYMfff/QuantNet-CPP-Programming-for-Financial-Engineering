...
double SD(std::vector<double> c, double r, double T)
{
	double sum_square = 0;
	double sum = 0;
	...
	return sqrt((sum_square - sum * sum / c.size()) / (c.size() - 1.0)) * exp(- r * T);
}

double SE(std::vector <double> c, double r, double T)
{
	return SD(c, r, T) / sqrt(c.size());
}
...
int main()
{
	...
	std::cout << "SD: " << SD(c, myOption.r, myOption.T) << endl;
	std::cout << "SE: " << SE(c, myOption.r, myOption.T) << endl;
