...
namespace NELSON
{
	namespace CAD
	{
		Circle::Circle() : center(0.0, 0.0), radius(1.0), Shape() // Default constructor
		{
		}

		Circle::Circle(const Point& p, const double& r) : center(p), radius(r), Shape() // Constructor with a CenterPoint and Radius.
		{
		}

		Circle::Circle(const Circle& c) : center(c.center), radius(c.radius), Shape(c) // Copy constructor
		{
		}
