...
namespace NELSON
{
	namespace CAD
	{
		Point::Point() : x(0.0), y(0.0), Shape() // default constructor
		{
		...
		Point::Point(const Point& pt) : x(pt.x), y(pt.y), Shape(pt) // copy constructor
		{
			cout << "A Point is copied.\n" << endl;
		}

		Point::Point(double value_x) : x(value_x), y(value_x), Shape()
		{
		}

		Point::Point(double value_x, double value_y) : x(value_x), y(value_y), Shape() // constructor that accepts x- and y-coordinates
		{
		...
		std::string Point::ToString() const
		{
			...
			String << "Point(" << x << ", " << y << ")" << s; // put into the String stream
